...
extern LJPEG_JSAMPLE * image_buffer;	/* Points to large array of R,G,B-order data */
...
write_JPEG_file (char * filename, int quality)
{
   ...
   */
  LJPEG_jpeg_set_quality(&cinfo, quality, TRUE /* limit to baseline-JPEG values */);
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    /* LJPEG_jpeg_write_scanlines expects an array of pointers to scanlines.
     * Here the array is only one element long, but you could pass
     * more than one scanline at a time if that's more convenient.
     */
    row_pointer[0] = & image_buffer[cinfo.next_scanline * row_stride];
    (void) LJPEG_jpeg_write_scanlines(&cinfo, row_pointer, 1);
  }
 ...
 * SOME FINE POINTS:
 *
 * In the above loop, we ignored the return value of LJPEG_jpeg_write_scanlines,
 * which is the number of scanlines actually written.  We could get away
 * with this because we were only relying on the value of cinfo.next_scanline,
 * which will be incremented correctly.  If you maintain additional loop
 * variables then you should be careful to increment them properly.
 * Actually, for output to a stdio stream you needn't worry, because
 * then LJPEG_jpeg_write_scanlines will write all the lines passed (or else exit
...
struct LJPEG_my_error_mgr {
  struct LJPEG_jpeg_error_mgr pub;	/* "public" fields */
...
LJPEG_my_error_exit (LJPEG_j_common_ptr cinfo)
{
  /* cinfo->err really points to a LJPEG_my_error_mgr struct, so coerce pointer */
  LJPEG_my_error_ptr myerr = (LJPEG_my_error_ptr) cinfo->err;
...
LJPEG_read_JPEG_file (char * filename)
{
   ...
   * struct, to avoid dangling-pointer problems.
   */
  struct LJPEG_my_error_mgr jerr;
  /* More stuff */
  ...
  LJPEG_jpeg_stdio_src(&cinfo, infile);

  /* Step 3: read file parameters with LJPEG_jpeg_read_header() */

  (void) LJPEG_jpeg_read_header(&cinfo, TRUE);
  /* We can ignore the return value from LJPEG_jpeg_read_header since
   ...
   * LJPEG_jpeg_read_header(), so we do nothing here.
   */
   ...
   * the data.  After LJPEG_jpeg_start_decompress() we have the correct scaled
